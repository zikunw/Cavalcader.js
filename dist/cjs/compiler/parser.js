...
exports.parseEdge = exports.parseNode = exports.parseExpression = exports.parseGraph = void 0;
const graph_1 = require("../graph/graph");
const graph_types_1 = require("../graph/graph-types");
const node_1 = require("../graph/node");
const edge_1 = require("../graph/edge");
function parseGraph(lexed) {
    // Check if first operator is a header
    if (!("type" in lexed[0] && lexed[0].type in graph_types_1.GraphType)) {
        throw new Error("Graph must start with a header");
    }
    const g = new graph_1.Graph(lexed[0].type);
...
function parseExpression(lexed, g) {
    let rest = lexed;
...
function parseNode(lexed) {
    ...
    const nInfo = lexed[0];
    const n = new node_1.Node(nInfo.label, nInfo.label, nInfo.shape); // HACK: id and label are the same for now
    return { node: n, rest: lexed.slice(1) };
}
exports.parseNode = parseNode;
// parse an edge
function parseEdge(lexed, prevNode) {
    ...
    const e = new edge_1.Edge(prevNode.id, nextNode.id, lexed[0].type, edge_length);
